...
namespace Andromeda.WebOrdering.Services
{
    public class OrderServices
    {
        ...
        public static bool PutOrder(string key, DomainConfiguration domainConfiguration, string siteId, string orderJson, out HttpStatusCode httpStatus, out string json)
        {
            ...
            {
                Global.Log.Error("PutOrder unhandled exception", exception);
        ...
        {
            Global.Log.Debug("GetPaymentProvider");
        ...
        {
            Global.Log.Debug("PutOrder paymentType=" + orderDetails.PaymentType);
        ...
        internal static bool SendOrder(DomainConfiguration domainConfiguration, OrderDetails orderDetails, List<ACSError> errors)
        {
            ...
            if (success)
            {
                ...
                foreach (string serverUrl in serverUrls)
                {
                    ...
                    Global.Log.Info("OrderServices.SendOrder returned json: " + json);
        ...
        private static bool SendConfirmationEmail(DomainConfiguration domainConfiguration, OrderDetails orderDetails)
        {
            ...
            {
                Global.Log.Info("OrderServices.SendConfirmationEmail Unable to send email - no email templates!");
            }
            else
            {
                ...
                if (domainConfiguration.TemplateEmails.TryGetValue(emailTemplateName, out confirmationEmailTemplate))
                {
                    ...
                    Global.Log.Info("CustomerServices.RequestResetPassword Email queued");
                }
                else
                {
                    Global.Log.Error("CustomerServices.SendConfirmationEmail Unable to send email - missing " + emailTemplateName + " email template");
                }
            }
        ...
        internal static void ProcessErrors(List<ACSError> errors, OrderDetails orderDetails)
        {
            ...
            string errorCode = "";
            Global.Log.Debug("ProcessErrors error0=" + (errors.Count > 0 && errors[0] == null ? "null" : "ok"));
            if (errors.Count > 0)
            {
                ...
                orderDetails.DomainConfiguration.TemplateEmails.Count > 0)
            {
                ...
                {
                    Global.Log.Debug("ProcessErrors sending order alert email");
        ...
        internal static string InjectEmailValues(string text, string errorCode, OrderDetails orderDetails)
        {
            ...
            if (orderPlaced != null)
            {
                DateTime orderPlacedDateTime = DateTime.Parse(orderPlaced.ToString());
        ...
        {
            bool success = true;
            Global.Log.Debug("PutOrder rollback payment paymentType=" + orderDetails.PaymentType);
            ...
            out OrderDetails orderDetails)
        {
            ...
            string surname = (string)customerDataElement["surname"];
            ...
            string org1 = (string)addressDataElement["org1"];
            string org2 = (string)addressDataElement["org2"];
            string prem1 = (string)addressDataElement["prem1"];
            string prem2 = (string)addressDataElement["prem2"];
            string roadNumber = (string)addressDataElement["roadNum"];
            string roadName = (string)addressDataElement["roadName"];
            string town = (string)addressDataElement["town"];
            string userLocality = (string)addressDataElement["userLocality"];
            string zipCode = (string)addressDataElement["zipCode"];
            if (zipCode == null)
            {
                zipCode = (string)addressDataElement["postcode"];
            }
            string directions = (string)addressDataElement["directions"];

            // Get the order price
            JObject pricingDataElement = (JObject)orderElement["pricing"];
            string priceAfterDiscount = (string)pricingDataElement["priceAfterDiscount"];
            ...
            if (deliveryChargeElement != null)
            {
                ...
                {
                    double deliveryCharge = OrderServices.PenceTextToDouble(deliveryChargeText);
        ...
        public static bool CheckOrderVouchers(string key, DomainConfiguration domainConfiguration, string siteId, string orderJson, out HttpStatusCode httpStatus, out string json)
        {
            ...
            {
                Global.Log.Error("CheckOrderVouchers unhandled exception", exception);
            ...
            out string resultJson)
        {
            ...
            if (success)
            {
                ...
                foreach (string serverUrl in serverUrls)
                {
                    ...
                    Global.Log.Info("OrderServices.SendCheckOrderVouchers returned json: " + json);
