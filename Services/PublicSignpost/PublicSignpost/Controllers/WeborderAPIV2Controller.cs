...
{
    public class WeborderAPIV2Controller : ApiController
    {
        [HttpGet]
        [ActionName("host")]
        public async Task<HttpResponseMessage> Host(string partnerId, string applicationId)
        {
            ...
            // Go get the hosts
            ResponseDetails responseDetails = await HostServices.GetHostsV2Async(new SignpostDataAccess(), dataTypes, partnerId, applicationId);
