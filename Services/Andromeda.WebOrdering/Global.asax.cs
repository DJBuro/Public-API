   ...
   //     internal static readonly ILog Log = LogManager.GetLogger(typeof(Global));
     ...
     //       log4net.Config.XmlConfigurator.Configure();
            ...
            try
            {
                ...
                Logger.Log.Info("Filewatcher Path=\"" + watcher.Path + "\" Filter=\"" + watcher.Filter + "\"");
                ...
                {
                    Logger.Log.Error("signalRHub setting missing or invalid");
                }
                else
                {
                    try
                    {
                        ...
                        Logger.Log.Info("Connected to signalRHub as connectionId " + Global.HubConnection.ConnectionId);
                    }
                    catch (Exception exception)
                    {
                        Logger.Log.Error("Failed to connect to SignalR", exception);
                    }
                }
        ...
        {
            Logger.Log.Debug("Getting mappings");
            ...
            lock (Helper.DomainConfiguration)
            {
                ...
                //string mappingFilename = ConfigurationManager.AppSettings["MappingFile"];
                //if (!String.IsNullOrEmpty(mappingFilename)  && File.Exists(mappingFilename))
                //{
        ...
        public static void LoadMappingFile(string mappingFilename)
        {
            using (StreamReader streamReader = new StreamReader(mappingFilename))
            {
                ...
                foreach (var mapping in mappings)
                {
                    ...
                    Logger.Log.Debug("Added mapping: domain=" + domainName + " appId=" + applicationId + " signpostServerText=" + signpostServerText);
                }
            }
        }
