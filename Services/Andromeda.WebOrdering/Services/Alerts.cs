...
namespace Andromeda.WebOrdering.Services
{
    public class Alerts
    {
        ...
        {
            Logger.Log.Info("Alerts.SendAlert");
            ...
            if (emailTemplate.To != null && emailTemplate.To.Length > 0)
            {
                ...
                Logger.Log.Info("Alerts.SendAlert Email queued");
            }
            else
            {
                Logger.Log.Info("Alerts.SendAlert Unable to send email - missing alertemail setting");
            }
        }

        internal static void SendFeedback(Email emailTemplate, string hostHeader, string subject, string body, string attachment)
        {
            Logger.Log.Info("Alerts.SendFeedback");
            ...
            if (emailTemplate.To != null && emailTemplate.To.Length > 0)
            {
                ...
                Logger.Log.Info("Alerts.SendFeedback Email queued");
            }
            else
            {
                Logger.Log.Info("Alerts.SendFeedback Unable to send email - missing feedbackemail setting");
            }
        }
    }
}