...
namespace Andromeda.WebOrdering.PaymentProviders
{
    public class PayPalPaymentProvider : IPaymentProvider
    {
        ...
        )
        {
           ...
           //         partnerReferenceProperty.Value = orderReference;
                ...
                if (success)
                {
                    ...
                    success = PayPalPaymentProvider.Initialise(domainConfiguration, orderDetails, out payToken);
                }
                ...
                if (success)
                {
                    ...
                    PaymentProviderHelper.SaveOrderToFile(orderJson, orderDetails.OrderId, domainConfiguration, pendingOrdersFolder);
            ...
            catch (PayPalException exception)
            {
                ...
                {
                    Logger.Log.Error("Paypal exception: " + ((ConnectionException)exception.InnerException).Response, exception);
                }
                else
                {
        ...
        private static bool Initialise(DomainConfiguration domainConfiguration, OrderDetails orderDetails, out string payToken)
        {
            bool success = true;
            payToken = "";
            ...
            ItemList itemList = new PayPal.Api.ItemList();
            ...
            foreach (OrderLine orderLine in orderDetails.OrderLines)
            {
                ...
                foreach (OrderLineTopping orderLineTopping in orderLine.AddToppings)
                {
                    ...
                    subTotal += orderLineTopping.Price;
                }
            ...
            orderDetails.Amount -= orderDetails.PaymentCharge;
                    ...
                    new Transaction
                    {
                        amount = new Amount()
                        {
                            ...
                            details = new Details
                            {
                                ...
                                shipping = (orderDetails.DeliveryCharge).ToString("0.00", CultureInfo.CurrentCulture),
                                tax = "0.00",
                                handling_fee = orderDetails.PaymentCharge.ToString("0.00", CultureInfo.CurrentCulture)
                            }
                        },
            ...
            {
                Logger.Log.Debug("DEBUG Create payment successful");
            ...
            {
                Logger.Log.Debug("ERROR Create payment failed");
            }
        ...
        private static string GetApprovalURL(PayPal.Api.Payment payment)
        {
        ...
        private static bool PaymentAccepted(DomainConfiguration domainConfiguration, string orderId, string token, string payerId, out string json)
        {
            ...
            try
            {
                ...
                Logger.Log.Debug("PayPalPaymentProvider ExecutePayment siteId:" + payPalOrderDetails.SiteId + " hostHeader:" + payPalOrderDetails.HostHeader);
        ...
        private static bool Execute(PayPalOrderDetails payPalOrderDetails)
        {
            ...
            if (success)
            {
                ...
                PayPal.Api.Payment payment = new PayPal.Api.Payment() { id = payPalOrderDetails.PayToken };
                PayPal.Api.Payment executedPayment = payment.Execute(apiContext, paymentExecution);
                ...
                if (!executedPayment.state.Equals("approved", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    Logger.Log.Error("PayPalPaymentProvider ExecutePayment PayPal execute returned a state of '" + executedPayment.state + "' for order id: " + payPalOrderDetails.OrderId);
                    success = false;
                }
            }
        ...
        private static bool PaymentTaken(PayPalOrderDetails payPalOrderDetails, out string json)
        {
            ...
            Logger.Log.Debug("PayPalPaymentProvider ExecutePayment successful");
            ...
            if (success)
            {
                ...
                Logger.Log.Debug("PayPalPaymentProvider ExecutePayment send order to store succeeded: httpStatus:" + httpStatus + " json:" + json);
            ...
            else
            {
                ...
                Logger.Log.Debug("PayPalPaymentProvider ExecutePayment send order to store failed");
            }
            ...
            {
                // Send an elert email
                Logger.Log.Error("PayPalPaymentProvider ExecutePayment Order.PutOrder failed with: " + json == null ? "no json" : json);
        ...
        {
            JToken paymentChargeElement = orderDetails.PaymentDataElement == null ? 0 : orderDetails.PaymentDataElement["paymentCharge"];
