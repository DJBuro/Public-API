...
namespace Andromeda.WebOrdering.PaymentProviders
{
    public class MercanetPaymentProvider : IPaymentProvider
    {
        public static bool PutMercanetPayment(string key, DomainConfiguration domainConfiguration, string siteId, string webOrderJson, out HttpStatusCode httpStatus, out string json)
        {
            ...
            try
            {
                ...
                if (success)
                {
                    ...
                    {
                        success = false;
                        Logger.Log.Debug("MerchanetCallback MerchantResponse invalid payment provider: " + orderDetails.PaymentProviderName);
                    }
                }
                ...
                if (success)
                {
                    ...
                    Logger.Log.Debug("DEBUG Initialise completed");
                    Logger.Log.Debug("DEBUG Initialise completed HTML=" + (html == null ? "NULL" : html));
                    if (html.Length > 0)
                    {
        ...
        private static string Initialise(decimal amount, string merchantId, string orderReference, DomainConfiguration domainConfiguration)
        {
            ...
            {
                Logger.Log.Error("Setting MercanetRequestExe is missing");
                return null;
            }
            ...
            {
                Logger.Log.Error("Setting MercanetPathFile is missing");
                return null;
            }
            ...
            Logger.Log.Debug("DEBUG Executing Mercanet request with arguments: " + arguments);
                ...
                using (Process process = new Process())
                {
                    ...
                    using (AutoResetEvent errorWaitHandle = new AutoResetEvent(false))
                    {
                            ...
                            errorWaitHandle.WaitOne(5000))
                        {
                            ...
                            {
                                Logger.Log.Error("Executing Mercanet request exe failed with return code " + process.ExitCode.ToString());
                                return null;
                            }
                        }
                        else
                        {
                            // Timed out.
                            Logger.Log.Error("Executing Mercanet request exe timed out");
                            return "";
                        }
                    }
                }
            ...
            {
                // Log error.
                Logger.Log.Error("Executing Mercanet request exe threw an exception", exception);
            }
        ...
        public static bool MerchanetCallback(string mercanetResponseString)
        {
            ...
            try
            {
                ...
                if (mercanetResponse != null)
                {
                    ...
                    Logger.Log.Debug("MerchanetCallback siteId:" + siteId + " hostHeader:" + hostHeader);
                    ...
                    if (success)
                    {
                        ...
                        {
                            success = false;
                            Logger.Log.Debug("MerchanetCallback MerchantResponse invalid payment provider: " + paymentProvider);
                        }
                    }
                    ...
                    if (success)
                    {
                        if (mercanetResponse.Code != "0" || mercanetResponse.ResponseCode != "00")
                        {
                            success = false;
                            errorCode = -1;
                            Logger.Log.Debug("MerchanetCallback MerchantResponse payment failed with code " + mercanetResponse.Code + " responseCode " + mercanetResponse.ResponseCode);
                        }
                    }
                    ...
                    if (success)
                    {
                        ...
                        Logger.Log.Debug("MerchanetCallback MerchantResponse payment successful");
                        ...
                        if (OrderServices.PutOrder("", domainConfiguration, siteId, "{ order: " + orderDetailsJson.ToString() + ", paymentType: '" + orderDetails.PaymentProviderName + "' }", out httpStatus, out json))
                        {
                            ...
                            Logger.Log.Debug("MerchanetCallback MerchantResponse send order to store succeeded: httpStatus:" + httpStatus + " json:" + json);
                        ...
                        else
                        {
                            ...
                            Logger.Log.Debug("MerchanetCallback MerchantResponse send order to store failed");
                        }
                        ...
                        {
                            // Send an elert email
                            Logger.Log.Error("MerchanetCallback Order.PutOrder failed with errorCode " + errorCode);
                    ...
                    if (success)
                    {
                        ...
                        if (hubResult != "OK")
                        {
                            ...
                            Logger.Log.Error("MerchanetCallback Hub call to OrderCompleted failed with '" + hubResult + "' for '" + mercanetResponseString + "'");
                            success = false;
                        }
                    }
                }
            ...
            {
                success = false;
                Logger.Log.Error("MerchanetCallback Unhandled exception", exception);
        ...
        {
            Logger.Log.Info("SendAlertEmail SendAlertEmail");
            ...
            Logger.Log.Info("SendAlertEmail SendAlertEmail Email queued");
        }
        ...
        private static MercanetResponse DecryptMerchantResponse(string merchanetResponse, string merchantId)
        {
            ...
            {
                Logger.Log.Error("DecryptMerchantResponse Setting MercanetResponseExe is missing");
                return null;
            }
            ...
            if (mercanetPathFile == null || mercanetPathFile.Length == 0)
            {
                Logger.Log.Error("DecryptMerchantResponse Setting MercanetPathFile is missing");
                return null;
            }
                ...
                using (Process process = new Process())
                {
                    ...
                    using (AutoResetEvent errorWaitHandle = new AutoResetEvent(false))
                    {
                        ...
                        if (process.WaitForExit(5000) &&
                            ...
                            errorWaitHandle.WaitOne(5000))
                        {
  //                          Logger.Log.Debug("DecryptMerchantResponse exitcode=" + process.ExitCode + " response=" + output);
 ...
 //                               Logger.Log.Debug("DecryptMerchantResponse decrypted: " + list);
 ...
 //                               Logger.Log.Debug("DecryptMerchantResponse extracted" + debug);
                                Logger.Log.Debug("DecryptMerchantResponse extracted " + list);
                                Logger.Log.Debug("DecryptMerchantResponse extracted code=" + mercanetResponse.Code + " responseCode=" + mercanetResponse.Code + " merchantId=" + mercanetResponse.MerchantId + " orderId=" + mercanetResponse.OrderId);
