...
function MenuHelper()
{
    ...
    self.addItemsToDealLine = function (dealLineWrapper)
    {
        ...
        for (var index = 0; index < dealLineWrapper.menuItemWrappers.length; index++)
        {
            ...
            if (menuItemWrapper.category1s !== undefined)
            {
                ...
                menuItemWrapper.category1s().sort(function (a, b)
                {
                    options || (options = []);

                    var menuItemA = options.filter(function (item)
                    {
                    ...
                    var menuItemB = options.filter(function (item)
                    {
    ...
    self.getAvailableTimes = function (times)
    {
        ...
        for (var availableTimesIndex = 0; availableTimesIndex < availableTimes.times.length; availableTimesIndex++)
        {
            ...
            {
                //if (availableTime.to == "00:00")
                //{
                //    if (availableTimesIndex + 1 < availableTimes.times.length)
                //    {
                //        var nextAvailableTime = availableTimes.times[availableTimesIndex + 1];
                //        if (nextAvailableTime.from == "00:00")
                //        {
                //            // We need to merge the two items
                //        }
                //    }
                //}
