var guiHelper =
{
    ...
    lockToOrderType: ko.observable(false),
    showView: function (viewName, newContentViewModel, forceShowSameView)
    {
        // DON'T CALL THIS DIRECTLY - GO THROUGH THE PAGE MANAGER
   ...
   //     alert((end.getTime() - start.getTime()) / 1000);
    ...
    {
        if (AndroWeb.Helpers.CartHelper.showCheckoutView())
        {
    ...
    showMenu: function (index)
    {
        ...
        AndroWeb.Helpers.CartHelper.isCheckoutMode(false);
    ...
    showMenuSection: function (sectionIndex, callback)
    {
        ...
        if (sectionIndex != undefined)
        {
            ...
            // Fix a layout problem when the sections sidebar is taller than the current section
            guiHelper.fixSectionHeight();
    ...
    fixSectionHeight: function()
    {
        // No menu sections sidebar in mobile mode
        if (guiHelper.isMobileMode())
        {
            $("#sectionsWrapper").height('auto');
            return;
        }

        // Little hack to fix a problem where there are a lot of menu sections i.e. very tall section sidebar
        // but where a menu section only has a couple of items in it
        var sectionHeight = $("#sectionsWrapper").height();
        var sectionsHeight = $("#sectionHeaders").height();

        // Is the section shorter than the sections sidebar?
        if (sectionsHeight > sectionHeight)
        {
            // Make the section the same height as the sections side bar
            $("#sectionsWrapper").height(sectionsHeight + 'px');
        }
        else
        {
            $("#sectionsWrapper").height('auto');
    ...
    downloadAndShowStoreMenu: function ()
    {
        ...
        try
        {
            ...
            // If both delivery and collection are disabled then you can look but not order
            if (settings.enableDelivery === false && settings.enableCollection === false)
            {
            ...
            //{
            //    this.lockToOrderType(lockToOrderType);

            //    viewModel.orderType(this.lockToOrderType());
                                                        ...
                                                        try
                                                        {
                                                            ...
                                                            (
                                                                ...
                                                                function ()
                                                                {
                                                                    ...
                                                                    {
                                                                        var path = queryStringHelper.pageName;
                                                                        if (queryStringHelper.menuSection != undefined)
                                                                        {
                                                                            path += "/" + queryStringHelper.menuSection;
                                                                        }

                                                                        viewModel.pageManager.showPage(path, true, undefined, false);
                                                                    }
                                                                    else
                                                                    {
                                                                        if (settings.isHomePageEnabled &&
                                                                            (queryStringHelper.voucherCode === undefined || queryStringHelper.voucherCode.length === 0) &&
                                                                            viewModel.isDineInMode() !== true)
                                                                        {
                                                                            // Show the home page
    ...
    renderMenu: function (menu, callback)
    {
        ...
        (
            ...
            function ()
            {
                ...
                // Build an index of toppings
                menuHelper.buildToppingLookup();
