    ...
    public static class DeliveryAreaViewModelExtensions
    {
        ...
        public static void UpdateFromViewModel(this Model.DeliveryAreaViewModel deliveryZone, IList<DeliveryArea> deliveryAreas)
        {
            if (deliveryZone != null)
            {
                ...
                {
                    List<string> deliveryAreaCodes = deliveryZone.DeliveryAreas.Split(new string[] { System.Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries).ToList().Where(s => (!string.IsNullOrEmpty(s.Trim()))).Select(s => s).ToList();
