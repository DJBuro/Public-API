...
namespace MyAndromeda.Web.Controllers.Api
{
    public class IBacsTelController : ApiController
    {
        ...
        public IBacsTelController(IMyAndromedaLogger logger, IDateServices dateServices, IWorkContext workContext, IOrderUpdateService orderUpdateService)
        {
        ...
        [HttpPost]
        public async Task<HttpResponseMessage> Post([FromBody] GprsPrinterIbacs model, string andromedaSiteId, string orderId)
        {
            ...
            try
            {
                ...
                try
                {
                    //int id = Convert.ToInt32(andromedaSiteId);

                    //if (this.gprsService.IsStoreGprsDevicebyAndromedaSiteId(id)) 
                    { 
                        //email is driven in here. 
                        //update status here, 
                        await this.orderUpdateService.UpdateOrderStatus(
                            Convert.ToInt32(andromedaSiteId), 
                            currentSite.ExternalSiteId, 
                            ramesesOrderId, acceptedOrder, model.Msg);
                    }
                }
                catch (Exception e)
                {
