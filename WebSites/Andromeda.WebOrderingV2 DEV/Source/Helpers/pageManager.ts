...
module AndroWeb {
    export class pageManager {
            ...
            if (Modernizr.history) 
            {
                ...
                (
                    ...
                    (ev: PopStateEvent): void => 
                    {
                        ...
                        // Fudge for chain level pages that are available for all stores
                        var siteMustBeSelected = true;
                        var pathParts: string[] = path.split("/");
                        if (pathParts.length > 0 && pathParts[0] === "") pathParts.splice(0, 1);
                        var rootPathPart = pathParts.length > 0 ? pathParts[0].toUpperCase() : "";

                        if (rootPathPart === "PLEASEWAIT" ||
                            rootPathPart === "FEEDBACK" ||
                            rootPathPart === "INFO" ||
                            rootPathPart === "PAGES")
                        {
                            siteMustBeSelected = false;
                        }

                        this.showPage(path, false, undefined, siteMustBeSelected);
                    }
                );
            }
        }
        ...
        public showPage(path: string, addToHistory: boolean = true, state: any = undefined, siteMustBeSelected: boolean = true) 
        {
            ...
            if (rootPathPart === 'MENU' && pathParts.length === 1) 
            {
                ...
                this.showPage('Menu/' + section.display.Name, true, undefined, true);
                return;
            }
            ...
            if (viewModel.selectedSite() === undefined &&
                (siteMustBeSelected === true || siteMustBeSelected === undefined))
            {
            ...
            else 
            {
                ...
                switch (rootPathPart) 
                {
                    ...
                    case "CHECKOUT": this.gotoCheckoutPage(pathParts, state); break;
                    ...
                    case "PAGES": this.gotoCmsPage(pathParts); break;
                    ...
                    case "UPSELL": this.gotoUpsellPage(pathParts, state); break;
        ...
        private gotoCmsPage(path: string[])
        {
            if (path.length > 1)
            {
                var pageName = path[1];
        ...
        private gotoDeliveryAddressPage(path: string[], state: any) 
        {
            ...
            if (this.canAccessCheckout(path)) 
            {
                ...
                {
                    viewHelper.showPopup(state);
                }
            }
        }

        private gotoUpsellPage(path: string[], state: any) 
        {
            if (state === undefined) 
            {
                guiHelper.showMenu(0);
        ...
        private canAccessCheckout(path: string[]) 
        {
            ...
            if (AndroWeb.Helpers.CartHelper.isCartEmpty()) 
            {
