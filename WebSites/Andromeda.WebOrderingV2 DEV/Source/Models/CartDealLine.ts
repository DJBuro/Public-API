...
module AndroWeb.Models
{
    export class CartDealLine
    {
        ...
        canBeCustomised(): boolean
        {
            ...
            var canBeCustomised: boolean = this.selectedAllowableMenuItemWrapper() != undefined && this.selectedAllowableMenuItemWrapper().name != textStrings.pleaseSelectAnItem;

            if (canBeCustomised)
            {
                if (this.selectedAllowableMenuItemWrapper().menuItemWrapper != undefined)
                {
                    canBeCustomised =
                        this.selectedAllowableMenuItemWrapper().menuItemWrapper.menuItem.OptTopIds.length > 0 ||
                        this.selectedAllowableMenuItemWrapper().menuItemWrapper.menuItem.DefTopIds.length > 0;
                }
            }

            return canBeCustomised;
        }
    }
}
