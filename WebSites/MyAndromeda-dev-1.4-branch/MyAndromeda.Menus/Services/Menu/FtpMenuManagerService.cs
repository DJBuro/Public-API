...
namespace MyAndromeda.Menus.Services.Menu
{
    public class FtpMenuManagerService : IFtpMenuManagerService
    {
        ...
        public void Dispose()
        {
            ...
            {
                logger.Error("FTP Menu Manager had a problem disposing the FTP service");
                logger.Error(e);
            }
        ...
        public FtpMenuContext DownloadMenu(SiteMenu siteMenu)
        {
            ...
            if (context.HasDownloadedMenu.GetValueOrDefault() && context.HasUnzippedMenu.GetValueOrDefault())
            {
                const string message = "Downloaded and unzipped the Rameses menu. Checking which is newer";
                this.notifier.Notify(message, true);
        ...
        public FtpMenuContext UploadMenu(int andromedaSiteId)
        {
            ...
            catch (Exception e)
            {
                ...
                while (exception != null)
                {
                    ...
                    {
                        ev.TransactionLog(new DatabaseUpdatedEventContext(ExpectedUserRoles.Administrator), string.Format("Exception for {0}: {1}", andromedaSiteId, exception.Message));
                        ev.TransactionLog(new DatabaseUpdatedEventContext(ExpectedUserRoles.Administrator), string.Format("Exception source {0}: {1}", andromedaSiteId, exception.Source));
                        ev.TransactionLog(new DatabaseUpdatedEventContext(ExpectedUserRoles.Administrator), string.Format("Exception source {0}: {1}", andromedaSiteId, exception.StackTrace));
