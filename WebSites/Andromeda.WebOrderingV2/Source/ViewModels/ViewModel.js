...
function ViewModel()
{
    ...
    self.changeSection = function ()
    {
        ...
        {
            showMenuSectionId = viewModel.selectedSection().Index;
        }
        
        if (section === undefined)
        {
            if (showMenuSectionId === undefined) showMenuSectionId = 0;

            section = viewModel.sections()[showMenuSectionId];
        }

        if (section !== undefined)
        {
            viewModel.pageManager.showPage('Menu/' + section.display.Name, true);
        }
    ...
    {
        self.onAddItemToCart(this, true);
        //viewModel.pageManager.showPage('CustomizeMenuItem', true, this);
    }
    ...
    self.addCustomItemToCart = function (context)
    {
        ...
        //viewModel.pageManager.showPage('CustomizeMenuItem', true, context);
    }
    ...
    self.onAddItemToCart = function (context, customize)
    {
        ...
        {
            var cartItem = new CartItem(context, menuItem, false);
            ...
            else
            {
                ...
                if (cartItem.toppings().length == 0)
                {
                    if (cartHelper.cart() !== undefined)
                    {
                        ...
                        var existingItems = cartItems.filter
                        (
                            function (existingCartItem)
                            {
                                var isTheCurrentItemId = existingCartItem.menuItem.Id === menuItem.Id;

                                return isTheCurrentItemId;
                            }
                        );
