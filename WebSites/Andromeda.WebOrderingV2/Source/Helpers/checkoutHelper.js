...
var checkoutHelper =
{
    ...
    initialiseCheckout: function ()
    {
        ...
        cartHelper.isCheckoutMode(true);
   ...
   //     checkoutHelper.showCheckoutSection(0);
    ...
    showCheckoutSection: function (sectionIndex, overrideNoSelect)
    {
        //var showCheckoutSection = checkoutHelper.checkoutSections()[sectionIndex];

        //if ((overrideNoSelect == undefined || overrideNoSelect == false) && showCheckoutSection.noSelect) return;
        ...
        //if (showCheckoutSection.id == 'checkoutOrderSummaryContainer')
        //{
        //    checkoutHelper.nextButton.displayText(textStrings.confirmOrder);
        //    checkoutHelper.nextButton.action = 'confirmorder';
        //    checkoutHelper.nextButton.isSendOrderButton(true);
        //}
        //else if (sectionIndex == checkoutHelper.checkoutSections().length - 1)
        //{
        //    checkoutHelper.nextButton.displayText(textStrings.placeOrder);
        //    checkoutHelper.nextButton.action = 'placeorder';
        //    checkoutHelper.nextButton.isSendOrderButton(false);
        //}
        //else
        //{
        //    checkoutHelper.nextButton.displayText(textStrings.next);
        ...
        //        // Bug 8151:GPRS\Andro Admin: Card payment option is displayed in web ordering site even after disabling the payment provider in a Andro admin
        //        if (checkoutSection.id == 'checkoutOrderSummaryContainer') {
        //            if (viewModel.siteDetails().paymentProvider.toUpperCase() == '')
        //                $('#' + 'checkoutSPayNowSection').css('display', 'none');
        //            else
        //                $('#' + 'checkoutSPayNowSection').css('display', 'block');
        //        }
        //    }
        //    else
        //    {
        ...
        //// Make sure the text is correct on the summary page
        //checkoutHelper.payTypeChanged();
    },
    refreshTimes: function ()
    {
        ...
        if (isOpenNow && checkoutHelper.times() != undefined && checkoutHelper.times().length > 0)
        {
            var firstTimeSlot = checkoutHelper.times()[0];
            firstTimeSlot.text = 'ASAP (' + firstTimeSlot.text + ')';
    ...
    showPaymentPicker: function ()
    {
        ...
        viewModel.pageManager.showPage('Payment', true);
    },
    payAtDoor: function ()
    {
        ...
        {
            alert('Sorry, we don\'t deliver to your postcode');
            return;
        }
    ...
    prePaymentOnlineCheck: function(callback)
    {
        ...
        viewModel.pageManager.showPage('PleaseWait', false, pleaseWaitViewModel);
    },
    mercuryPayment: function()
    {
        ...
        (
            ...
            function ()
            {
                viewModel.pageManager.showPage('MercuryPayment', true);
            },
            function ()
            {
    ...
    datacashPayment: function()
    {
        ...
        (
            ...
            function ()
            {
                viewModel.pageManager.showPage('DatacashPayment', true);
            },
            function ()
            {
                guiHelper.showCheckoutAfterError();
    ...
    mercanetPayment: function()
    {
        ...
        (
            ...
            function (json)
            {
                ...
                viewModel.pageManager.showPage('MercanetPayment', true);
            },
            function ()
            {
                guiHelper.showCheckoutAfterError();
    ...
    sendOrderToStore: function ()
    {
        ...
        (
            function()
            {
                ...
                {
                    // Generate the order JSON
                    var orderDetails = checkoutHelper.generateOrderJson(cartHelper.cart().vouchers());
    ...
    sendOrderToStoreCallback: function(result)
    {
        ...
        {
            viewModel.telemetry.sendTelemetryData('Sales', 'Checkout', 'Failed', result.errorCode);
        ...
        else
        {
            ...
            viewModel.pageManager.showPage('OrderResult', true, orderAcceptedViewModel);
        }
    },
    generateOrderJson: function (vouchers)
    {
        ...
        //var loyaltyValues = // checkoutHelper.checkoutDetails().loyaltyPoints();
        ...
        var orderDetails =
        {
            ...
            order:
            {
                ...
                pricing:
                {
                    ...
                    priceBeforeDiscount: cartHelper.cart().subTotalPrice(),                    
                    discounts: [],
                    priceAfterDiscount: cartHelper.cart().totalPrice(),
                    finalPrice: cartHelper.cart().totalPrice()
                },
                orderLines:
                [
                ],
        ...
        if (settings.simulateBadOrder)
        {
            ...
            orderDetails.order.pricing.priceBeforeDiscount = 1.111111111;
        }
        else
        {
                ...
                var discount =
                    {
                        ...
                        discountAmount: cartHelper.cart().discountAmount(),
                        initialDiscountReason: cartHelper.cart().discountName()
                    };
            ...
            for (var voucherCodeIndex = 0; voucherCodeIndex < vouchers.length; voucherCodeIndex++)
            {
                ...
                if (voucherCode.IsValid)
                {
                    var discount =
                    {
                        ...
                        discountTypeAmount: voucherCode.EffectValue,
        ...
        for (var index = 0; index < cartHelper.cart().deals().length; index++)
        {
            ...
            if (cartDeal.isEnabled())
            {
                ...
                for (var dealLineIndex = 0; dealLineIndex < cartDeal.cartDealLines().length; dealLineIndex++)
                {
                    ...
                    var cat1 = helper.findById(cartDealLine.selectedMenuItem.Cat1, viewModel.menu.Category1);
                    var cat2 = helper.findById(cartDealLine.selectedMenuItem.Cat2, viewModel.menu.Category2);

                    var orderLine =
                    {
                        productId: cartDealLine.selectedMenuItem.Id,
                        menuSection: 'Deals',
                        cat1: cat1 != undefined ? cat1.Name : '',
                        cat2: cat2 != undefined ? cat2.Name : '',
                        quantity: 1,
                        price: 0, //cartDealLine.finalPrice,
                        name: cartDealLine.cartItem().name,
                        orderLineIndex: orderLineNumber,
                        lineType: 0, // Normal item (deal line)
                        instructions: cartDealLine.instructions,
                        person: cartDealLine.person,
                        inDealFlag: true,
                        addToppings:
                        [
                        ],
                        ...
                        (
                            {
                                ...
                                quantity: topping.cartQuantity(),
                                name: topping.topping.ToppingName == undefined ? topping.topping.Name : topping.topping.ToppingName,
                                price: topping.price,//topping.finalPrice,
                                itemPrice: topping.itemPrice
                            }
                        );
                    }
    ...
    {
        // Make sure wanring is enabled (might have been disabled during payment if need to be diverted to an external website)
                                            ...
                                            if (data.data.length == 1)
                                            {
                                                ...
                                                if (voucher != undefined)
                                                {
                                                    ...
                                                    else if (voucher.IsValid)
                                                    {
                                                        ...
                                                        cartHelper.cart().vouchers(data.data);
    ...
    useAddress: function()
    {
        ...
        if (checkoutHelper.checkoutDetails.rememberAddress)
        {
            ...
            accountHelper.save(checkoutHelper.useAddressFinished);
        }
        else
        {
            checkoutHelper.useAddressFinished();
        }
    },
    useAddressFinished: function()
    {
        viewModel.pageManager.showPage("Checkout", true);
    },
