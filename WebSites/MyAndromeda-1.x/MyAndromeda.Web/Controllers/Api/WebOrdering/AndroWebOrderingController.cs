...
namespace MyAndromeda.Web.Controllers.Api.WebOrdering
{
    public class AndroWebOrderingController : ApiController
    {
        ...
        public IEnumerable<WebSiteConfigurations> Read(int webOrderingWebsiteId)
        {
            ...
            {
                result.SocialNetworkSettings.FacebookSettings = new SocialNetworkSiteSettings();
            }
        ...
        public async Task<HttpResponseMessage> Update(WebSiteConfigurations configuration, int webOrderingWebsiteId)
        {
            ...
            try
            {
                ...
                this.notifier.Notify("The website changes have been saved.");
            }
            catch (Exception)
            {
                this.notifier.Error("The website had errors while saving.");
            }
        ...
        public async Task<HttpResponseMessage> Preview(WebSiteConfigurations configuration, int webOrderingWebsiteId)
        {
            ...
            var result = await this.previewPublishService.PreviewWebOrderingSettingsAsync(configuration);

            if (!result)
            {
                this.notifier.Error("There was an error updating the preview website. Please try again.");
                return Request.CreateErrorResponse(HttpStatusCode.InternalServerError, new Exception("Something went wrong in Preview"));
            }
