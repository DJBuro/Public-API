...
namespace MyAndromeda.Web.Areas.Reporting.Controllers
{
    public class OrdersController : Controller 
    {
        ...
        public OrdersController(IOrderService orderService, ISpreadsheetDocumentExport spreadsheetExport, ITranslator translator, INotifier notifier, IAuthorizer authorizer, IAcsCustomerDataService acsCustomerDataService, ICustomerOrdersDataService customerOrderService, IDateServices dateServices) 
        {
        ...
        public ActionResult ReadCustomerHistory([DataSourceRequest] DataSourceRequest request, Guid? customerId, Guid? acsOrderId) 
        {
            ...
            if (!customerId.HasValue)
            {
                ...
                data = this.customerOrderService.List().Where(e => e.CustomerID == customer.ID);
            }
            else 
            {
                data = this.customerOrderService.List().Where(e => e.CustomerID == customerId.Value);
            }
            
            return Json(data.ToDataSourceResult(request, e => e.ToViewModel(dateServices)));
        }
        ...
        public ActionResult Read([DataSourceRequest] DataSourceRequest request, int fromYear, int fromMonth, int fromDay, int toYear, int toMonth, int toDay) 
        {
            ...
            //{
            //    return ReadExcel(request, fromYear, fromMonth, fromDay, toYear, toMonth, toDay);
            //}

            var filter = new FilterQuery()
            {
                FilterFrom = new DateTime(fromYear, fromMonth, fromDay),
                FilterTo = new DateTime(toYear, toMonth, toDay)
            };

            var data = orderService.GetListData(filter).OrderByDescending(e=> e.TimeStamp);

            //DataSourceRequest is expecting a flatter model, which doesn't mean i cant redirect it to the ORM models to more effectively query the database.
