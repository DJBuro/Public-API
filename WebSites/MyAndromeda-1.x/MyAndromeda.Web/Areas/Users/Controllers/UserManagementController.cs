...
namespace MyAndromeda.Web.Areas.Users.Controllers
{
    public class UserManagementController : Controller
    {
        ...
        {
            if (!this.authorizer.Authorize(Permissions.ListUsers)) 
            {
        ...
        public async Task<ActionResult> CreatePost(CreateEditUserViewModel viewModel)
        {
            ...
            if(this.currentChain.Available)
                return RedirectToAction("AddUserToChain", new { chainId = this.currentChain.Chain.Id, userId = model.Id });
            else
        ...
        {
            if (!this.authorizer.Authorize(Permissions.CreateUsers))
            {
        ...
        public ActionResult EditRoles(int userId)
        {
            ...
            if(!authorizer.Authorize( Permissions.AssignAdministratorRole))
            {
                availableRoles = availableRoles.Where(e => e.Name != ExpectedRoles.Administrator);
            }

            if (!authorizer.Authorize(Permissions.AssignSuperAdministratorRole)) 
            {
                availableRoles = availableRoles.Where(e => e.Name != ExpectedRoles.SuperAdministrator);
            }
        ...
        public ActionResult EditRolesPost(int userId, ViewModels.UserRoleViewModel viewModel)
        {
            ...
            var selectedRoles = 
                viewModel.SelectedRoles == null ? Enumerable.Empty<IUserRole>() : availableRoles.Where(e=> viewModel.SelectedRoles.Contains(e.Id));
        ...
        {
            if (!authorizer.Authorize(Permissions.ResetPassword)) 
            {
        ...
        {
            if (!authorizer.Authorize(Permissions.RemoveUserFromChain))
            {
        ...
        {
            if (!authorizer.Authorize(Permissions.RemoveUserFromStore))
            {
                this.notifier.Error(translator.T("You do not have permissions to remove this user"));
                return new HttpUnauthorizedResult();
            }
