...
namespace MyAndromeda.Menus.Jobs
{
    ...
    public class SyncFtpUpJob : Job
    {
        ...
        private static void LogFtpStart(IEnumerable<IFtpEvents> ftpEvents)
        {
            ...
            {
                ev.TransactionLog(new DatabaseUpdatedEventContext(ExpectedRoles.Administrator), "Upload ftp Job started");
            }
        }
        ...
        private static void LogFtpTaskStatus(SiteMenu[] sitesToFetch, IEnumerable<IFtpEvents> ftpEvents)
        {
            ...
            foreach (var ev in ftpEvents)
            {
                ev.TransactionLog(new DatabaseUpdatedEventContext(ExpectedRoles.Administrator), string.Format("Ftp upload bits to process: {0}", sitesToFetch.Length));
            }
        }
        ...
        {
            var eventContext = new DatabaseUpdatedEventContext(ExpectedRoles.Administrator);
        ...
        private static void LogException(SiteMenu siteMenu, IEnumerable<IFtpEvents> ftpEvents, Exception ex)
        {
            foreach (var ev in ftpEvents)
            {
                ev.TransactionLog(new DatabaseUpdatedEventContext(ExpectedRoles.Administrator), string.Format("{0}: exception", siteMenu.AndromediaId));

                while (ex != null)
                {
                    ev.TransactionLog(new DatabaseUpdatedEventContext(ExpectedRoles.Administrator), string.Format("Exception for {0}: {1}", siteMenu.AndromediaId, ex.Message));
                    ev.TransactionLog(new DatabaseUpdatedEventContext(ExpectedRoles.Administrator), string.Format("Exception source {0}: {1}", siteMenu.AndromediaId, ex.Source));
                    ev.TransactionLog(new DatabaseUpdatedEventContext(ExpectedRoles.Administrator), string.Format("Exception source {0}: {1}", siteMenu.AndromediaId, ex.StackTrace));
        ...
        private static void LogCompletionOfTask(SiteMenu siteMenu, IEnumerable<IFtpEvents> ftpEvents, IEnumerable<IAccessDatabaseEvent> events)
        {
            ...
            foreach (var ev in ftpEvents)
            {
                ev.TransactionLog(new DatabaseUpdatedEventContext(ExpectedRoles.Administrator), string.Format("Task Completed: {0}", siteMenu.AndromediaId));
            }
        }
    }
}