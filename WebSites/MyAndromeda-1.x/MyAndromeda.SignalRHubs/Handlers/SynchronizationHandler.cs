...
namespace MyAndromeda.SignalRHubs.Handlers
{
    ...
    public class SynchronizationHandler : ISynchronizationEvent
    {
        ...
        public void Skipping(SyncronizationContext context)
        {
            ...
            hubContext.Clients.Group(ExpectedRoles.SuperAdministrator).ping(string.Format("SynchronizationHandler skipped ping: {0}", DateTime.UtcNow));
            hubContext.Clients.Group(ExpectedRoles.SuperAdministrator).skippedSynchronization(context);
        }
