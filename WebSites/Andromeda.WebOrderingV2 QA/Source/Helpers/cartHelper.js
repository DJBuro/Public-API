...
function CartHelper()
{
    ...
    (
        {
            ...
            totalPriceBeforeLoyalty: ko.observable(),
            ...
            preLoyaltySubTotalPrice: ko.observable(),
            ...
            deliveryCharge: 0,
            displayDeliveryCharge: ko.observable(''),
            cardCharge: 0,
            displayCardCharge: ko.observable(''),
            displayCardChargePayment: ko.observable('')
        }
    );
    ...
    this.refreshCart = function (applyCardCharge)
    {
        ...
        // Loyalty
        cartHelper.cart().preLoyaltySubTotalPrice(cartDetails.totalPrice);

        // loyalty discount has been done here ... 
        loyaltyHelper.refreshLoyalty(cartDetails);

        // Delivery charge
        cartHelper.refreshDeliveryCharges(cartDetails);

        // Card charge
        cartHelper.refreshCardCharges(cartDetails, applyCardCharge);
        ...
        {
            cartDetails.checkoutEnabled = cartHelper.cart().preLoyaltySubTotalPrice() >= settings.minimumDeliveryOrder;
        }
    ...
    this.refreshCartLineItems = function (cartDetails)
    {
        ...
        for (var index = 0; index < cartHelper.cart().cartItems().length; index++)
        {
            ...
            item.isEnabled(menuHelper.isItemEnabledForCart(item.itemWrapper));
    ...
    this.refreshDeliveryCharges = function (cartDetails)
    {
        // Is there a delivery charge?
        if (settings.deliveryCharge === undefined || viewModel.orderType() !== 'delivery')
        {
            self.removeDeliveryCharge(cartDetails);
            return;
        }
        ...
        if (settings.optionalFreeDeliveryThreshold !== undefined)
        {
            ...
            if (cartDetails.totalPrice < settings.optionalFreeDeliveryThreshold)
            {
                ...
                self.applyDeliveryCharge(cartDetails);
            }
            else
            {
    ...
    this.applyDeliveryCharge = function (cartDetails)
    {
        // The delivery charge is a decimal in MyAndromeda
        var deliveryChargePence = settings.deliveryCharge;

        // Add delivery charge
        cartHelper.cart().deliveryCharge = deliveryChargePence;
        cartHelper.cart().displayDeliveryCharge(helper.formatPrice(deliveryChargePence));

        // Add the delivery charge to the total price
        cartDetails.totalPrice = cartDetails.totalPrice + deliveryChargePence;
    };
    this.removeDeliveryCharge = function (cartDetails)
    {
        // Add delivery charge
        cartHelper.cart().deliveryCharge = 0;
        cartHelper.cart().displayDeliveryCharge('');
    };
    this.refreshCardCharges = function (cartDetails, applyCardCharge)
    {
            ...
            settings.cardCharge > 0)
        {
            ...
            cartHelper.cart().cardCharge = settings.cardCharge;
        ...
        {
            cartHelper.cart().displayCardCharge(textStrings.checkCardCharge.replace('{AMOUNT}', helper.formatPrice(settings.cardCharge)));
            cartHelper.cart().displayCardChargePayment(textStrings.checkCardChargePayment.replace('{AMOUNT}', helper.formatPrice(settings.cardCharge)));
        }
        else
        {
            cartHelper.cart().displayCardCharge('');
            cartHelper.cart().displayCardChargePayment('');
        }
    };
    ...
    this.clearCart = function ()
    {
        ...
        cartHelper.cart().dataCashPaymentDetails(undefined);
    ...
    this.checkout = function ()
    {
        ...
        if (settings.customerAccountsEnabled && !accountHelper.isLoggedIn())
        {
            ...
            viewModel.pageManager.showPage('Login', true, new LoginViewModel(true), false);
        }
