...
var acsapi =
{
    ...
    getSite: function (siteId, gotMenuVersion, callback, statusCheck, errorCallback)
    {
        ...
        (
            {
                url: viewModel.serverUrl + "/sites/" + siteId + "?key=x&gotMenuVersion=" + gotMenuVersion + '&statusCheck=false',
                type: 'GET',
        ...
        (
            {
                ...
                dataType: 'json',
                cache: false,
                data: '{ "amount": "' + AndroWeb.Helpers.CartHelper.cart().totalPrice() + '", "returnUrl": "' + window.location.href + '/Done.html"}'
            }
        )
        .done
        (
            function (data, textStatus, jqXHR)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        AndroWeb.Helpers.CartHelper.cart().mercuryPaymentId(data.paymentId)
    ...
    putPayPalPayment: function (siteId, order, callback, errorCallback)
    {
    ...
    {
        if (settings.debugAlwaysAcceptOrder === true)
        {
            callback({ hasError: false, errorMessage: undefined, errorCode: undefined, orderNumber: 12345, isProvisional: true, data: undefined, paymentReset: false });
    ...
    checkForError: function (data, alwaysReturnAnError, result)
    {
        ...
        if (data != undefined && data.errorCode != undefined && data.errorCode.length > 0)
        {
            ...
            {
                AndroWeb.Helpers.CartHelper.cart().dataCashPaymentDetails().reference = '';
            }
