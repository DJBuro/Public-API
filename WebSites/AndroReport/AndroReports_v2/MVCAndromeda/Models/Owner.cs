            ...
            else
            {
                foreach (var country in CountriesAndStores.Keys)
                {
                    CountriesAndStores[country].Sort((string storeName1, string storeName2) => 
                        orderHow * (
                        (long)(Stores[storeName1].StoreData[orderWhat-1][0]==null?
                        orderHow==-1?Int64.MinValue:Int64.MaxValue
                        :Stores[storeName1].StoreData[orderWhat-1][0])
                        ).CompareTo(
                        (long)(Stores[storeName2].StoreData[orderWhat-1][0]==null?
                        orderHow==-1?Int64.MinValue:Int64.MaxValue
                        :Stores[storeName2].StoreData[orderWhat-1][0]))
                        );
                }
            }
        }
