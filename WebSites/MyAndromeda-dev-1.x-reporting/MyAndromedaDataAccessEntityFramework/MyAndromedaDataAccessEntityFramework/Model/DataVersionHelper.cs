...
namespace MyAndromeda.Data.Model
{
    public static class DataVersionHelper
    {
        public static int GetNextDataVersionForEntity(this AndroAdminDbContext entitiesContext)
        {
            return GetNextDataVersion(entitiesContext);
        }

        public static int GetNextDataVersion(AndroAdminDbContext entitiesContext)
        {
            ...
            int newVersion = -1;
            using (var sqlConnection = new SqlConnection(entitiesContext.Database.Connection.ConnectionString)) 
            { 
