...
namespace MyAndromeda.Data.DataWarehouse.Services.Orders
{
    public class OrderReportingDataService : IOrderReportingDataService 
    {
        ...
        public OrderReportingDataService(DataWarehouseDbContext dbContext)
        {
        ...
        {
            var data = OrderHeaderTable
                .Include(x=>x.OrderStatu)
                .Include(x=> x.Customer)
                .Include(x=> x.Customer.Contacts)
                .Include(x=> x.OrderLoyalties)
                .Include(x=> x.UsedVouchers)
                .Where(query);
        ...
        public IEnumerable<SummaryByDay<decimal>> GetTotalOrdersByDay(Expression<Func<OrderHeader, bool>> query)
        {
            ...
            var groupedData = data.GroupBy(e =>
                new
                {
                    ...
                    Year = e.Key.Year,
                    OrderCount = e.Count(),
                    Total = e.Sum(item => item.FinalPrice + item.DeliveryCharge),
                    MinPrice = e.Min(item => item.FinalPrice + item.DeliveryCharge),
                    MaxPrice = e.Max(item => item.FinalPrice + item.DeliveryCharge),
                    AvgPrice = e.Average(item => item.FinalPrice + item.DeliveryCharge),
                    CollectionCount = e.Count(item => item.OrderType == "COLLECTION"),
                    DeliveryCount = e.Count(item => item.OrderType == "DELIVERY"),
                    //DiscountSum = e.SelectMany(m => m.OrderDiscounts).Sum(d => d.DiscountAmount),
        ...
        public IEnumerable<SummaryOfLineItem> GetOrderLineSummary(Expression<Func<OrderLine, bool>> query)
        {
            ...
            var groupedData = data
                ...
                .GroupBy(e => new { e.ProductID, e.Description, e.DisplayCategory, e.Cat1, e.Cat2, e.Price })
                .Select(e => 
                    new { 
