...
namespace MyAndromeda.Data.DataWarehouse.Services.Tokens
{
    public class CustomerTokenizer : ITokenProvider
    {
        private static readonly Type customerType = typeof(CustomerModel);
        ...
        public IEnumerable<ReplacablePart> ProcessFor<T>(TokenContext context) 
            where T : class
        {
            var t = typeof(T);
            if (t != customerType)
