    ...
    public class CustomerController : Controller
    {
        ...
        public ActionResult ListValue([DataSourceRequest]DataSourceRequest request)
        {
            ...
            var data = this.customerDataService
                ...
                .Where(e=> e.OrderHeaders.Any(o => o.ExternalSiteID == this.workContext.CurrentSite.ExternalSiteId))
                .Select(e => new
                {
                    Id = e.ID,
                    ...
                    Title = e.Title,
                    TotalOrders = e.OrderHeaders
                        .Where(o => o.ExternalSiteID == this.workContext.CurrentSite.ExternalSiteId)
                        .Count(),
                    TotalValue = e.OrderHeaders
                        .Where(o => o.ExternalSiteID == this.workContext.CurrentSite.ExternalSiteId)
                        .Sum(order => (decimal?)order.FinalPrice + order.DeliveryCharge) ?? 0,
                    AvgOrderValue = e.OrderHeaders
                        .Where(o => o.ExternalSiteID == this.workContext.CurrentSite.ExternalSiteId)
                        .Average(order => (decimal?)order.FinalPrice + order.DeliveryCharge) ?? 0,
                    LastOrderTime = e.OrderHeaders
                        .Where(o => o.ExternalSiteID == this.workContext.CurrentSite.ExternalSiteId)
                        .Max(order => (DateTime?)order.TimeStamp)
                }).ToArray();
